...
ll query(ll s,ll e,ll i,ll l,ll r)
{
    ...
    {
        ll mid=(l+r)>>1;
        return query(s,e,2*i,l,mid) + query(s,e,2*i+1,mid+1,r);
    }
}
int main()
{
    ...
    {
        cin>>t;
        ll lo=1,hi=n; //here we use binary search to find the index of element in original array.Here time complexity is O(n*log(n)*log(n)).We can improve more.
        while(lo<=hi)
        {
            ll mid=(lo+hi)>>1;
            if(query(1,mid,1,1,n)>=t)
                hi=mid-1;
            else lo=mid+1;
        }
        cout<<v[lo]<<" ";
