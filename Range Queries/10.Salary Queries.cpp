...
int main()
{
    ...
    f(i,0,n)
    {
        ll idx=lower_bound(all(t),u[i])-t.begin()+1;
        update(idx,1);
    }
    for(auto [x,y,z]:query)
    {
        if(x)
        {
            y--;    //0-based indexing,be careful
            ll idx=lower_bound(all(t),u[y])-t.begin()+1;
            update(idx,-1);
            idx=lower_bound(all(t),z)-t.begin()+1;
            update(idx,1);
            u[y]=z;
        }
        else
        {
            ll idx1,idx2;
            idx1=lower_bound(all(t),y)-t.begin()+1;
            if(binary_search(all(t),z))
                idx2=lower_bound(all(t),z)-t.begin()+1; //if its found then ok else seek just smaller element
            else idx2=lower_bound(all(t),z)-t.begin();  
            // idx2 = upper_bound(all(t),z)-t.begin();  //or it can be directly done like this
            cout<<sum(idx1,idx2)<<"\n";
        }
    }
    return 0;
